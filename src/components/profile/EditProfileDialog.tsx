...
const NITRO_PIN_ENC = "c2hvaGFpbDA5"; // base64 encoded, private
const getNitroPin = () => {
...
export const EditProfileDialog = ({ profile, open, onOpenChange }: EditProfileDialogProps) => {
  ...
  const [pendingNitro, setPendingNitro] = useState(false);
  ...
  React.useEffect(() => {
    ...
    // If already has nitro theme, consider unlocked
    if ((profile as any)?.profile_theme === 'nitro') {
  ...
  const handleNitroPinSubmit = () => {
    ...
    if (pin === getNitroPin()) {
      ...
      setNitroPinInput("");
      setPendingNitro(false);
      toast({ title: "Nitro unlocked! ⚡", description: "Black & white verified badge enabled + GIF cover" });
    } else {
      toast({ title: "Wrong PIN", description: "Incorrect Nitro PIN", variant: "destructive" });
    }
  };
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] p-0 gap-0 rounded-[24px]">
          ...
          <ScrollArea className="max-h-[70vh] px-6">
            <div className="space-y-6 pb-6">
              <div>
                <Label>Banner {profileTheme === 'nitro' && <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-black text-white">GIF allowed</span>}</Label>
                <p className="text-[11px] text-muted-foreground mb-2">{profileTheme === 'nitro' ? "Nitro: GIF supported • 1500x500" : "Recommended: 1500×500px"}</p>
                ...
                <input ref={bannerFileRef} type="file" accept={profileTheme === 'nitro' ? "image/*,image/gif" : "image/*"} className="hidden" onChange={handleBannerFileSelect} />
              </div>
                ...
                {isVerified && (
                  <div className="rounded-[18px] border border-zinc-200 dark:border-zinc-800 bg-gradient-to-br from-zinc-50 via-white to-zinc-50 dark:from-zinc-900 dark:via-black dark:to-zinc-900 p-4 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div><div className="text-sm font-semibold">Verified Themes</div><div className="text-[11px] text-muted-foreground">Only verified can use • Nitro locked</div></div>
                      <span className="ml-auto text-[10px] px-2.5 py-1 rounded-full bg-black text-white font-bold">NITRO LOCKED</span>
                    </div>
                    <div className="rounded-[14px] bg-background/80 border border-border/50 p-3">
                      ...
                      {profileTheme === 'nitro' && isNitroUnlocked && <div className="mt-3 text-[11px] p-2 rounded-lg bg-black text-white text-center">⚡ Nitro active • Verified badge is now B&W • GIF cover enabled</div>}
                    </div>
                  </div>
                )}
      ...
      {/* Nitro PIN Dialog */}
      <Dialog open={showNitroPinDialog} onOpenChange={(o) => { setShowNitroPinDialog(o); if (!o) { setNitroPinInput(""); setPendingNitro(false); } }}>
        <DialogContent className="max-w-sm rounded-[20px]">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setShowNitroPinDialog(false); setNitroPinInput(""); setPendingNitro(false); }}>Cancel</Button>
