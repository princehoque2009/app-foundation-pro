  ...
  return (
    <>
      <div className={cn("relative pb-4", isVerifiedTheme && `profile-theme-${theme}`)}>
        ...
        <div className={cn("profile-banner relative w-full overflow-hidden bg-muted h-[180px] sm:h-[220px] md:h-[260px] lg:h-[300px] xl:h-[320px] lg:rounded-[24px] lg:max-w-[1024px] lg:mx-auto lg:border lg:shadow-sm", theme === 'yellow' ? "lg:border-amber-300/50 border-amber-200 shadow-[0_0_0_1px_rgba(251,191,36,0.2),0_8px_24px_-8px_rgba(251,191,36,0.3)] lg:shadow-[0_0_0_1px_rgba(251,191,36,0.2),0_8px_24px_-8px_rgba(251,191,36,0.3)]" : "lg:border-border/50", theme === 'mono' && "")}>
          {isOwner ? (
            <CoverPhotoUploader userId={userId} currentCoverUrl={profile?.cover_photo_url} isOwner />
          ) : profile?.cover_photo_url ? (
            <img src={optimizeCloudinaryUrl(profile.cover_photo_url, theme === 'yellow' ? "c_fill,ar_3:1,g_auto,w_1200" : "c_fill,ar_3:1,g_auto,w_1200")} alt={`${profile?.display_name || profile?.username || "User"} cover`} className={cn("w-full h-full object-cover pointer-events-none select-none", theme === 'mono' && "grayscale")} draggable={false} onContextMenu={(e) => e.preventDefault()} />
          ) : (
            <div className={cn("absolute inset-0", theme === 'yellow' ? "bg-gradient-to-br from-amber-100 via-yellow-50 to-orange-50 dark:from-amber-950/30 dark:via-yellow-950/20 dark:to-orange-950/20" : theme === 'mono' ? "bg-gradient-to-br from-zinc-100 via-zinc-50 to-white dark:from-zinc-800 dark:via-zinc-800 dark:to-zinc-900" : "bg-gradient-to-br from-muted via-background to-muted")} />
          )}
