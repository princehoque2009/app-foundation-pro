  ...
  return (
    <>
      <div className="relative pb-4">
        {/* FIXED: smaller banner on desktop */}
        <div className="relative w-full overflow-hidden bg-muted h-[180px] sm:h-[220px] md:h-[260px] lg:h-[300px] xl:h-[320px] lg:rounded-[24px] lg:max-w-[1024px] lg:mx-auto lg:border lg:border-border/50 lg:shadow-sm">
          {isOwner ? (
            <CoverPhotoUploader userId={userId} currentCoverUrl={profile?.cover_photo_url} isOwner />
          ) : profile?.cover_photo_url ? (
            <img
              src={optimizeCloudinaryUrl(profile.cover_photo_url, "c_fill,ar_3:1,g_auto,w_1200")}
        ...
        <div className="px-4 sm:px-6 max-w-2xl mx-auto">
          ...
          <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1, duration: 0.3 }} className="mt-3 text-center">
            <div className="flex items-center justify-center gap-2 flex-wrap">
              <h1 className={cn("text-xl sm:text-2xl font-semibold tracking-tight", effects.hasRainbowName ? "bg-gradient-to-r from-red-500 via-yellow-500 via-green-500 via-blue-500 to-purple-500 bg-clip-text text-transparent animate-gradient-x" : "text-foreground")}>{profile?.display_name || profile?.username}</h1>
